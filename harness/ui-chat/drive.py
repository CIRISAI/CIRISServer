...
# CLIENT 0.5.223+ (CIRISAgent#1179): send-traces is a REQUIRED Yes/No on the
# consent screen — `trace_consent_yes` / `trace_consent_no`, neither
# pre-selected, and `btn_next` stays DISABLED until one is chosen.
# `toggle_trace_opt_in` is gone from that screen. `TRACE_OPT_IN=no` answers No.
TRACE_ANSWER = "trace_consent_no" if os.environ.get("TRACE_OPT_IN", "yes").lower() == "no" else "trace_consent_yes"
WIZARD_CHOICES = ("age_band_adult", TRACE_ANSWER)
...
def _load_nav_hops() -> list[dict[str, list[str]]]:
    """Every screen's click chain, DERIVED from the pinned client's sources.

    Client 0.5.224 replaced the sidebar with the circles shell (CIRISAgent#1181),
    and #64 moved hops again. The client's own `testing/gate/nav_map.py` parses
    `CirclesNav.kt` / `EpistemicNav.kt` / `CIRISApp.kt`, so a re-home is picked
    up by re-pinning, never by editing a table here (a hand-written map drifts
    silently: a wrong hop looks exactly like a screen that failed to compose).
    `run.sh` sparse-checks-out the client at the version pyproject pins into
    `CIRIS_CLIENT_SRC`. Both builds are loaded: whether an agent is attached is a
    RUNTIME fact of the node under test, and it changes the chains.
    """
    src = Path(os.environ.get("CIRIS_CLIENT_SRC", Path(__file__).parent / ".client-src"))
    if not (src / "testing" / "gate" / "nav_map.py").exists():
        raise SystemExit(
            f"no client sources at {src} — run.sh checks them out at the pinned version "
            f"(or set CIRIS_CLIENT_SRC to a CIRISClient checkout)"
        )
    sys.path.insert(0, str(src))
    from testing.gate import nav_map  # type: ignore

    return [nav_map.build(has_agent=False), nav_map.build(has_agent=True)]


NAV_HOPS: list[dict[str, list[str]]] = []


def goto(ui: Ui, expect_screen: str, secs: int = 60) -> None:
    """Walk the shell to `expect_screen` and CONFIRM the landing.
    ...
    if ui.screen() == expect_screen:
        return
    if not NAV_HOPS:
        NAV_HOPS.extend(_load_nav_hops())
    chains = [h[expect_screen] for h in NAV_HOPS if expect_screen in h]
    if not chains:
        raise UiError(f"{ui.name}: the pinned client places no screen {expect_screen!r}")
    present = set(ui.tags())
    chain = next((c for c in chains if c[0] in present), chains[0])
    log(ui.name, f"nav   {ui.screen()} -> {expect_screen} via {' > '.join(chain)}")
    for tag in chain:
        hop_deadline = time.time() + 20
        while tag not in set(ui.tags()) and time.time() < hop_deadline:
            time.sleep(1)
        ui.click(tag)
        time.sleep(1.5)
    deadline = time.time() + secs
    while time.time() < deadline:
        ...
        time.sleep(2)
    raise UiError(
        f"{ui.name}: {' > '.join(chain)} did not reach {expect_screen!r} "
...
def first_run(ui: Ui, username: str, password: str, device_name: str) -> None:
    ...
    # FIRST RUN STARTS AT THE LOGIN CHOOSER, NOT THE WIZARD (client #1055; the
    # agent's runner does the same: CIRISAgent tools/qa_runner web_ui
    # `wait_for_setup`). Choose local sign-in, then POLL to a deadline for
    # `Setup`: 'Startup' is a transient splash, and one look after 3 s used to
    # read a still-composing Login as "wizard complete" — the wizard never ran.
    # A Login that shows `input_username` AFTER the click is an already-claimed
    # node, which is a precondition failure, not a pass.
    ...
    while time.time() < deadline:
        ...
        if screen == "Setup":
            break
        tags = set(ui.tags())
        if screen == "Login" and not clicked_local and "btn_local_login" in tags:
            log(ui.name, "first run starts at the Login chooser — selecting local sign-in")
            ui.click("btn_local_login")
            clicked_local = True
        elif screen == "Login" and clicked_local and "input_username" in tags and "btn_next" not in tags:
            raise UiError(
                f"{ui.name}: local sign-in shows a LOGIN form, not the Setup wizard — this "
                f"node is already configured, so first run cannot be exercised"
            )
        time.sleep(0.5)
    if ui.screen() != "Setup":
        raise UiError(f"{ui.name}: Setup wizard did not appear within 45s (screen={ui.screen()!r})")
...
def add_contact(ui: Ui, code_or_id: str, expect_listed: str) -> None:
    """Add the peer as a contact — through the form, not the API.

    `code_or_id` is what a person types: the CODE they were handed, or a bare
    federation id if they already have one. The field takes either.

    A code is the stranger path and the reason this works at all. An earlier
    revision tried to admit the peer first through NetworkPeers' `btn_add_peer`
    — but that surface posts `/v1/setup/claim-remote` with `cohort=self`, which
    is claiming a node as YOUR OWN, not admitting someone else's. Pasting the
    handed-over code here is the person-facing path, and it carries what the
    directory would otherwise have to supply.

    `expect_listed` is the identity the contact should resolve TO — checked
    separately from what was typed, so a form that accepts a code and resolves it
    to nothing cannot pass.
    """
    goto(ui, "Contacts")
    log(ui.name, f"adding contact from {code_or_id[:28]}… expecting {expect_listed}")
    if ui.api_port:
        held = directory_of(ui.api_port)
        present = expect_listed in held
    ...
    ui.type_into("input_contacts_add_key", code_or_id)
    ...
    while time.time() < deadline:
        ...
        if expect_listed in blob or "contacts_list" in blob:
            log(ui.name, f"contact listed (resolved to {expect_listed})")
            return
        time.sleep(5)
    raise UiError(
        f"{ui.name}: contact {expect_listed!r} never appeared in the list. The node "
...
def main() -> int:
    ...
    try:
        ...
        # bare id. There is no separate "admit the peer" step, and the surface
        ...
        stage("contact:a_adds_b", lambda: add_contact(a, b_code, b_owner))
        stage("consent:a", lambda: consent(a))
        stage("contact:b_adds_a", lambda: add_contact(b, a_code, a_owner))
