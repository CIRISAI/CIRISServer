...
XFAIL_family_on_b="CIRISServer#646 (no Family replication round; fixed on this branch) + CIRISPersist#910: B is added AFTER create, so B's node needs the GROWN record, and supersede_group_row re-stamps admitted_at without re-indexing the wire while put_family is a plain INSERT at a peer holding the founding record"
XFAIL_family_file_listed_on_b="downstream of family_on_b (CIRISPersist#910): a family row's audience is the family's members' nodes as B's node folds them, and B's node does not hold the grown record naming B"
XFAIL_family_file_opened_on_b="downstream of family_file_listed_on_b (CIRISPersist#910); REQUIRED once the family plane converges (#647 asks for the bytes stage to be REQUIRED)"
