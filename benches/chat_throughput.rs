...
//! set, a burst (1000 total — CIRISPersist#804 landed in persist v41; see the
...
//!
//! 100/900 is the intended shape and, since persist v41 (CIRISPersist#804),
//! the default again: local authorship takes its own write door and is no
//! longer metered as a stranger's burst. The ceiling detection below stays —
//! a refusal at `peer_burst` on the owner's own sends is the regression.
...
impl Cfg {
    fn from_env() -> Self {
        ...
        Cfg {
            // ── BACK ABOVE THE WRITE CEILING (persist v41, CIRISPersist#804) ──
            //
            // 0.5.197 sized this at 50 + 100 because every chat send placed its
            // row through the one write door, which charged persist's per-peer
            // quota on `row.attesting_key_id` — the HUMAN, since edge v20 — and
            // refused the owner's own burst at ~250 (measured 243, 248 locally;
            // ~344 in CI). v41 gives the write an origin axis: `share` and the
            // local put both take `put_attestation_authored`, which is the
            // node's own ceiling and never a peer bucket. So this measures the
            // intended shape again, and the `peer_burst` ceiling recorder
            // below is now the regression detector, not the expected outcome.
