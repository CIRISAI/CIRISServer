...
async fn admit_node_seed_replicates_and_peer_roots_end_to_end() {
    ...
    for e in [&engine_a, &engine_b] {
        e.register_federation_key({
            let mut r = to_persist(&a1_anchor);
    ...
    // CIRISPersist#547 — RESOLVED in persist v24.1.0. `adopt_scrub_upgrade` used
    // to mutate `federation_keys` in place without maintaining
    // `signed_wire_index`, so A advertised a Key ref it could not serve until its
    // next restart. A characterization pin lived here asserting that MISS, plus a
    // post-rebuild success proving the diagnosis was index staleness rather than a
    // serialization skew.
    //
    // The pin did its job the way a good one does: it started FAILING the moment
    // persist fixed the mutators, which is the signal to delete it. Restored to
    // the direct expectation — A serves its own record bytes, no rebuild needed.
    let bytes = a_bridge
...
/// registration (was lenient in v16). This software fixture supplies the minimal
/// structurally-valid evidence the persist gate checks for presence; real chain
/// validation is the consumer's job (CC 4.2.2.1), not this admission gate.
