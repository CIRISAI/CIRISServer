...
// **RETIRED with `client/` (CIRISServer#471).**
//
// `gate_localization_bundles_are_byte_identical_mirrors` lived here. It
// existed because this repo carried FOUR copies of the locale bundle and a
// merge of two green branches touching different copies produced a state
// neither branch was in — a violation the PR-triggered guard could not see,
// because the breakage is created by an event its trigger does not cover.
//
// There is ONE bundle now, and this repo does not carry it: it arrives inside
// `ciris-client[node]`, byte-identical by construction because it is a single
// artifact rather than four files someone has to keep in step. The class of
// defect is gone, not merely unchecked.
//
// Recorded rather than silently deleted: a release gate that disappears
// without explanation reads, later, as coverage someone dropped.
