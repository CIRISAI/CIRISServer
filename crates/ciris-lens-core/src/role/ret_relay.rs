...
impl LensCore {
    ...
    pub async fn ret_relay(
        ...
        let ret_config = ReticulumTransportConfig {
            ...
            // CIRISEdge#508 item (d). A lens relay is not the canonical that
            // wedged, but it carries the same replication planes, and
            // leviculum's 256 default is documented as sized for "small std
            // platforms" with servers expected to override. Edge clamps this
            // into 16..=65_536, so the value cannot escape its DoS bound.
            //
            // Capacity is HEADROOM, not a cure: #508's own priority order puts
            // it LAST, behind the completion-mirror eviction and the single
            // mutex that actually stall the consumer (leviculum#56-#59). It
            // buys time in front of a stall; it does not prevent one.
            control_channel_capacity: Some(4096),
            // CIRISEdge#499 — a lens relay must stay reachable. `false` would
            // make it point-to-point only, and a silent reachability change on
            // upgrade is precisely what edge warns this flag can cause.
