...
| **False/forced halt** (griefing DoS). | Fail-secure RAISE path: replicate-to-peers-**first**, then latch (`HUMANITY_ACCORD_HALT`), then `exit(42)`; `check_halt_gate` blocks every future boot. Recovery needs a verified `accord:lifecycle:active` — never an operator restart — by either route: **live-roster** (M-of-N incl. ≥1 original genesis holder, `accord_reactivate`, needs the DB) or **offline release token** (M-of-N over the BAKED genesis, `accord_release`, needs only two files in `home`; CIRISServer#347). The token is bound to `{node_id, halt_invocation_id, halt_payload_sha256, latch_id}`, so it is not replayable to another node, another halt, or a later latch of the same halt; every attempt is journaled. | Recovery is now O(1 ceremony) + a file copy per node rather than O(nodes) physical visits, but it is still **not automatic and not in-band**: someone must carry the token to each dark host. The offline route needs ≥M *founding* seats (the baked roster); a family that has rotated past its founders must use the live-roster route, which needs a healthy DB — so a node whose DB is also broken and whose founders' keys are gone stays down by design (CC 4.2.3). A halt still carries **no TTL and no cohort scope**: a mistaken halt reaches every node that receives it, and nothing expires it. |
